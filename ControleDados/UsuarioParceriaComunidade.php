<?php
	...
	Class MailjetMailPartner
	{
		public function emailPostPartner($nameUser, $emailUser, $comunidade)
		{
		    $clientP = new \Mailjet\Client('308f549a732b5fbd97b43f0b0c6f3054', 'e1400ff82ace61bd86a0a57b1998510b');
		    $emailP = [
		      'FromName'     => 'Academiq',
		      'FromEmail'    => 'marcos.junior@uniriotec.br',
		      'Text-Part'    => '',
		      'Html-Part'    => '<h2>Olá, ' . $nameUser .'</h2><br><p>Você foi adicionado à comunidade' . $comunidade .' como parceiro</p><br><p>           Equipe Academiq</p>',
		      'Subject'      => 'Academiq - Notificação de Parceria',
		      'Recipients'   => [['Email' => $emailUser]],
		    ];

		    $retP = $clientP->post(Resources::$Email, ['body' => $email]);
			...
			while($row = mysqli_fetch_assoc($result)) {
				...
				if ($titulo == "aluno") {
					...
					if (mysqli_num_rows($result2) > 0) {
						...
						$emailSender= new MailjetMailPartner();
						$emailSender->emailPostPartner($nomeUsuario, $email, $row);
